...
def sa_wrapper(data_path="./data"):
    ...
    train_SA = simple_average(train_probas, train[0])
    ...
    for csv in sorted(os.listdir(data_path)):
        if ".csv" in csv:
            if "train" in csv:
                ...
                train_SA.to_csv(os.path.join(data_path, args.exp, args.exp + "_train_SA.csv"), index=False)
            elif ("dev" in csv) or ("val" in csv):
                os.remove(os.path.join(data_path, csv))
...
def main(path, gt_path="./data/"):
    ...
    for csv in sorted(os.listdir(path)):
        ...
        if ".csv" in csv:
            if "train" in csv:
                train.append(pd.read_csv(os.path.join(path, csv)))
                train_probas[csv[:-4]] = pd.read_csv(os.path.join(path, csv)).proba.values
    ...
    while (delta > 0.0001):
        ...
        if loop > 0:
            while len(dev) > 5:
                ...
                column_numbers = [x for x in range(train_probas.shape[1])]  # list of columns' integer indices
                column_numbers.remove(drop)
                train_probas = train_probas.iloc[:, column_numbers]
        ...
        ### SIMPLE AVERAGE ###
        train_SA = simple_average(train_probas, train[0], power=1, normalize=True)
        ...
        ### POWER AVERAGE ###
        train_PA = simple_average(train_probas, train[0], power=2, normalize=True)
        ...
        train_SX = simple_average(train_probas, train[0], weights_dev)
        ...
        # Prepare Next Round
        train = train_or + [train_SA, train_PA, train_RA, train_SX]
        ...
        train_probas = pd.concat([df.proba for df in train], axis=1)
    ...
    os.makedirs(os.path.join(path, args.exp), exist_ok=True)
    ...
    train_SX.to_csv(os.path.join(path, args.exp, "FIN_train_" + args.exp + "_" + str(loop)) + ".csv", index=False)
    dev_SX.to_csv(os.path.join(path, args.exp, "FIN_dev_seen_" + args.exp + "_" + str(loop) + ".csv"), index=False)
    test_SX.to_csv(os.path.join(path, args.exp, "FIN_test_seen_" + args.exp + "_" + str(loop) + ".csv"), index=False)
    test_unseen_SX.to_csv(os.path.join(path, args.exp, "FIN_test_unseen_" + args.exp + "_" + str(loop) + ".csv"), index=False)
